...
$DOCKER_OPTS $IMG_NAME /bin/bash -ex -c 'echo "Starting building postgres binaries" \
    ...
    && cp --path /lib/*/libz.so.1 /lib/*/liblzma.so.5 /usr/lib/libossp-uuid.so.16 /usr/lib/*/libxml2.so.2 /usr/lib/*/libxslt.so.1 ./lib/ \
    && cp --path /lib/*/libssl.so.1.0.0 /lib/*/libcrypto.so.1.0.0 ./lib || cp --path /usr/lib/*/libssl.so.1.0.0 /usr/lib/*/libcrypto.so.1.0.0 ./lib/ \
    && if [ "$ICU_ENABLED" = true ]; then cp --path --no-dereference /usr/lib/*/libicudata.so* /usr/lib/*/libicuuc.so* /usr/lib/*/libicui18n.so* ./lib/; fi \
    && if [ -n "$POSTGIS_VERSION" ]; then cp --path --no-dereference /lib/*/libjson-c.so* /usr/lib/*/libsqlite3.so* ./lib/ ; fi \
